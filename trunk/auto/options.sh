...
SRS_UTEST=RESERVED
SRS_SUPPORT_CDN_CLUSTER=RESERVED
# tcmalloc
SRS_GPERF=RESERVED
# gperf memory check
...
function parse_user_option() {
    case "$option" in
        ...
        --with-cdn-cluster)             SRS_SUPPORT_CDN_CLUSTER=YES ;;
        ...
        --without-cdn-cluster)          SRS_SUPPORT_CDN_CLUSTER=NO  ;;
...
SRS_AUTO_CONFIGURE="--prefix=${SRS_PREFIX}"
    ...
    if [ $SRS_SUPPORT_CDN_CLUSTER = YES ]; then SRS_AUTO_CONFIGURE="${SRS_AUTO_CONFIGURE} --with-cdn-cluster"; else SRS_AUTO_CONFIGURE="${SRS_AUTO_CONFIGURE} --without-cdn-cluster"; fi
